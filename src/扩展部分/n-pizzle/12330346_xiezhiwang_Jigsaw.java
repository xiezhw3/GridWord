...
public class Jigsaw {
    ...
    //    if (jNode.getNodeDepth() > maxDepth) {
    //        maxDepth += 5;
    ...
    public boolean BFSearch() throws IOException {
        ...
        while (openList.size() > 0) {
            this.currentJNode = openList.firstElement();
            ...
            Vector<JigsawNode> validNeighborNodes = findFollowJNodes(this.currentJNode);
            for (int i = 0; i != validNeighborNodes.size(); i++) {
                this.openList.addElement(validNeighborNodes.get(i));
            }
        }    
    ...
    public boolean ASearch() throws IOException{
        ...
        PrintWriter pw = new PrintWriter(new FileWriter(filePath));
        //PrintWriter pw2 = new PrintWriter(new FileWriter("nodeDepth.txt", true));
        ...
        while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
            ...
            if (this.currentJNode.equals(this.endJNode)){
            //    pw2.println("The deepth is: " + currentJNode.getNodeDepth());
                isCompleted = true;
                this.calSolutionPath();
                break;
            }
            ...
            if (searchedNodesNum == maxNodesNum) {
            //    pw2.println("not found----The deepth is: " + currentJNode.getNodeDepth());
            }
        }
    ...
    private void estimateValue(JigsawNode jNode) {
        int estValue = 2 * estimateValueWrongDis(jNode) +
                           estimateValueWrongNums(jNode) +
                           estimateValueConti(jNode) -
                       2 * estimateValueRightNum(jNode) -
                           estimateValueNextRightNum(jNode);
        //if (jNode.getParent() != null)
            //estValue += jNode.getParent().getEstimatedValue();
        jNode.setEstimatedValue(estValue);
    }
    
    /**
     * 代价估计函数，计算放错位的数码与其正确位的距离之和
     * D(n) 代表后续节点数码与其正确位置的距离之和
     * @param jNode - 要计算代价估计值的节点；此函数返回
     * 节点在这个估价函数的估价值
     * */
    private int estimateValueWrongDis(JigsawNode jNode) {
        int dis = 0;
            ...
            if(jNode.getNodesState()[index] != endJNode.getNodesState()[index]) {
                for (int j = 1; j != dimension * dimension; j++) {
                    if (jNode.getNodesState()[index] == endJNode.getNodesState()[j]) {
                        rowx = (index - 1) / dimension;
                        colx = (index - 1) % dimension;
                        rowy = (j - 1) / dimension;
                        coly = (j - 1) % dimension;
                        dis += Math.abs(rowx - rowy) + Math.abs(colx - coly);
                        break;
                    }
                }
            }
     ...
     * 后续节点不正确的数码个数。
     * */
    private int estimateValueWrongNums(JigsawNode jNode) {
        int s = 0; //放错位的数码个数
        int dimension = JigsawNode.getDimension();
        for(int index = 1 ; index < dimension * dimension; index++){
            if(jNode.getNodesState()[index]!=endJNode.getNodesState()[index])
     ...
     * @param jNode - 要计算代价估计值的节点；此函数会改变该节点的estimatedValue属性值。
     */
    private int estimateValueConti(JigsawNode jNode) {
    ...
    private int estimateValueRightNum(JigsawNode jNode) {
        int s = 0; // 对应位正确的数码个数
        int index = 1;
        int dimension = JigsawNode.getDimension();
        ...
        for(; index < dimension * dimension; index++){
            if(jNode.getNodesState()[index] == endJNode.getNodesState()[index]) {
                s++;
            }
        }
    ...
    private int estimateValueNextRightNum(JigsawNode jNode) {
        int s = 0; //后续节点正确的码个数
        int dimension = JigsawNode.getDimension();
        for(int index = 1 ; index < dimension * dimension; index++){
            if(jNode.getNodesState()[index] + 1 == jNode.getNodesState()[index + 1]) {
